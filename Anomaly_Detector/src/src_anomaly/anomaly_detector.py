...
dim_list = k_mean_dim.outlier_dim_list
final_list = []
...
final_anomaly_dict = defaultdict(dict)
	...
	for dim in powerset[item]:
		...
		for alist in parent_list:
			...
			if percent_hit >= THRESHOLD_PERCENT:
				final_anomaly_dict[dim_com][key] = percent_hit

'''
for item in range(len(dimension_list)):
...
print(final_anomaly_dict)
for dim_com in final_anomaly_dict:
	for key in final_anomaly_dict[dim_com]:
		val = float(final_anomaly_dict[dim_com][key])
		dim_str = str(dim_com)
		key_str = str(key)

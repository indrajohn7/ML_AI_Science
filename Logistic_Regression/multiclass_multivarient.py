...
def main():
	...
	with open("multi_class.txt", 'r') as fp:
		for line in fp:
			line = line.rstrip('\n')
	...
	for key_val in multi_class:
		...
		theta_list = compute_grad_variable(x_list, temp_y_list)
		multi_class[key_val] = theta_list
	...
	while(True):
		...
		class_node = -1
		for key_val in multi_class:
			theta_list = multi_class[key_val]
			res = get_sigmoid_trend_of_input(input_list, theta_list)
			if (res > prob):
				prob = res
				class_node = key_val

		print("Param node belongs to Class: %d with probability MAX[P(y=i|x;<theta>)]: %f" % (class_node, prob))
